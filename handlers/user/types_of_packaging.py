...
async def edit_useful_information(message: Message, state: FSMContext):
    ...
    await state.set_state(BotContentEditStates.edit_types_of_packaging)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_types_of_packaging)
...
async def edit_bag_tape(message: Message, state: FSMContext):
    ...
    if message.from_user.id not in ADMIN_USER_ID:
        ...
        return
    await message.answer("Введите новый текст, используя разметку HTML.")
    await state.set_state(BotContentEditStates.edit_bag_tape)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_bag_tape)
async def update_info(message: Message, state: FSMContext):
...
async def edit_box_bag_tape(message: Message, state: FSMContext):
    ...
    if message.from_user.id not in ADMIN_USER_ID:
        ...
        return
    await message.answer("Введите новый текст, используя разметку HTML.")
    await state.set_state(BotContentEditStates.edit_box_bag_tape)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_box_bag_tape)
async def update_info(message: Message, state: FSMContext):
...
async def edit_wooden_sheathing_bag_tape(message: Message, state: FSMContext):
    ...
    if message.from_user.id not in ADMIN_USER_ID:
        ...
        return
    await message.answer("Введите новый текст, используя разметку HTML.")
    await state.set_state(BotContentEditStates.edit_wooden_sheathing_bag_tape)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_wooden_sheathing_bag_tape)
async def update_info(message: Message, state: FSMContext):
...
async def edit_wooden_corners_bag_tape(message: Message, state: FSMContext):
    ...
    if message.from_user.id not in ADMIN_USER_ID:
        ...
        return
    await message.answer("Введите новый текст, используя разметку HTML.")
    await state.set_state(BotContentEditStates.edit_wooden_corners_bag_tape)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_wooden_corners_bag_tape)
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text,
...
async def edit_pallet_crate(message: Message, state: FSMContext):
    ...
    if message.from_user.id not in ADMIN_USER_ID:
        ...
        return
    await message.answer("Введите новый текст, используя разметку HTML.")
    await state.set_state(BotContentEditStates.edit_pallet_crate)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_pallet_crate)
async def update_info(message: Message, state: FSMContext):
...
async def edit_pallet_with_a_solid_wooden_box(message: Message, state: FSMContext):
    ...
    if message.from_user.id not in ADMIN_USER_ID:
        ...
        return
    await message.answer("Введите новый текст, используя разметку HTML.")
    await state.set_state(BotContentEditStates.edit_pallet_with_a_solid_wooden_box)


# Обработчик текстовых сообщений (для админа, чтобы обновить информацию)
@router.message(BotContentEditStates.edit_pallet_with_a_solid_wooden_box)
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text,
